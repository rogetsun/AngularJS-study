/**
 * Created by uv2sun on 15/5/15.
 */

angular.module('treeData', []).factory('TreeData', function () {
    /**
   ...
   *   }
     ...
     * @param singleSelectMust 只能单选
     * @param isFolderSelectable 父节点是否可选
     * @param cbIsSame {function(Object, Object):boolean}
     * @constructor
     */

    function TreeData(tree, singleSelectMust, isFolderSelectable, cbIsSame) {
         ...
         * @private
         */
        this._check = function (item, checked, broadcastable) {
        ...
        this.check = function (item, checked) {
            if (!isFolderSelectable || angular.isUndefined(isFolderSelectable)) {
                console.log(item);
                if (item.items && item.items.length > 0) {
                    console.log('cannot select folder! it has ' + item.items.length + ' children');
                    item.checked = !item.checked;
                } else {
                    if (singleSelectMust && this.checkedList.length > 0) {
                        while (this.checkedList.length > 0) {
                            var last = this.checkedList.pop();
                            this.uncheck(last);
                        }
                    }
